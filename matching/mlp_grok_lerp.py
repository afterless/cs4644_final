...
if __name__ == "__main__":
    ...
    for p in t.arange(0, 1, step=0.1):
        modelC_dict = {}
        for (n, _), p_a, p_b in zip(
            modelC.named_parameters(), modelA.parameters(), modelB.parameters()
        ):
        ...
        with t.inference_mode():
            for data, target in tqdm(test_loader):
                ...
                i += 1
        test_loss /= i
        acc = 100.0 * correct / len(test_loader.dataset)  # type: ignore
        print(
            "Average Test Loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)\n".format(
                test_loss, correct, len(test_loader.dataset), acc  # type: ignore
            )
        )
